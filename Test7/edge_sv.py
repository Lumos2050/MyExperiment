...
def canny_weight(msf_init, pan_init, seg_edge, low = 50, high = 150):
    ...
    return weighted_image, normalize_seg_edge, mse_msf, mse_pan
...
class ResNet18(nn.Module):
    ...
    def __init__(self, inplane):
        ...
        self.blk3_1 = ResBlk(inplane, 256, stride=1)
...
class SqueezeBodyEdge(nn.Module):
    ...
    def __init__(self, inplane):
        ...
        self.flow_make = nn.Sequential(
            ...
            nn.BatchNorm2d(2),
            nn.ReLU(inplace=True))
        self.conv_5to4 = nn.Sequential(
            nn.Conv2d(inplane + inplane//4, inplane, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(inplane),
            nn.ReLU(inplace=True))
...
#init的归一化在canny函数里面
        ...
        weighted_image, seg_edge, mse_msf, mse_pan = canny_weight(msf_init, pan_init, seg_edge, low = 50, high = 150)#包含归一化(B, 16, H, W)
        ...
        fine_edge = self.conv_5to4(torch.cat([seg_edge, weighted_image], dim = 1))#(B, 4, H/2, W/2)
...
if __name__ == '__main__':
    ...
    x = torch.randn((5, 192, 16, 16),dtype=torch.float32).cuda()
    msf_init = torch.randn((5, 48, 32, 32),dtype=torch.float32).cuda()
    pan_init = torch.randn((5, 48, 32, 32),dtype=torch.float32).cuda()
