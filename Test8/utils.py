...
#定义训练方法
def train_model(model, train_loader, optimizer, epoch, lamda1=0.01, lamda2=0.01):
    ...
    for step, (ms, fu, pan, label, _) in enumerate(loop):
        ...
        loss = F.cross_entropy(output, label.long()) + lamda1 * torch.sum(mse_msf) + lamda2 * torch.sum(mse_pan)
        loss.backward()
        optimizer.step()
