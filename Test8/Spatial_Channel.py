...
class ResidualGroup(nn.Module):
    ...
    def __init__(   self,
        ...
        self.conv6 = nn.Sequential(nn.Conv2d(dim*2, dim*2, 3, 1, 1), nn.BatchNorm2d(dim*2), nn.LeakyReLU(inplace=True))
        self.conv9 = nn.Sequential(nn.Conv2d(dim*4, dim*4, 3, 1, 1), nn.BatchNorm2d(dim*4), nn.LeakyReLU(inplace=True))
    ...
    def forward(self, msf, fu, pan, x_size):
        ...
        z6 = rearrange(z6, "b (h w) c -> b c h w", h = H1, w = W1)
        z6 = self.conv6(z6)
...
class DAT(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_init_edge = nn.Sequential(nn.Conv2d(embed_dim,embed_dim, 3, 1, 1),nn.BatchNorm2d(embed_dim),nn.LeakyReLU())
        ...
        #self.conv_after_body9 = nn.Sequential(nn.Conv2d(embed_dim*4, embed_dim*4, 3, 1, 1), nn.BatchNorm2d(embed_dim*4), nn.LeakyReLU(inplace=True))
        #self.conv_then = nn.Sequential(nn.Conv2d(embed_dim*2, embed_dim, 3, 1, 1), nn.BatchNorm2d(embed_dim), nn.LeakyReLU(inplace=True))
        #self.conv_last = nn.Sequential(nn.Conv2d(embed_dim, num_feat, 3, 1, 1), nn.BatchNorm2d(num_feat), nn.Sigmoid())
    ...
    def forward(self, msf, fu, pan):
        ...
        x = self.conv_after_body6(self.forward_features(msf, fu, pan))
        #x = self.conv_then(x)
        #x = self.conv_last(x)
...
if __name__ == '__main__':
    ...
    Output, msf_init, pan_init, = model(msf, fu, pan)
    #torch.Size([1, 4, 32, 32])
    print(Output.shape, msf_init.shape, pan_init.shape)
"""