...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    没有swin
    """
    def __init__(self, dim, num_heads, split_size=[8,16], drop=0., attn_drop=0.):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.split_size = split_size
...
class Process_Spatial_FFN(nn.Module):
    ...
    输入输出都加了Lnorm
    """
    def __init__(self, dim, num_heads, split_size=[8,16],expansion_factor=2, proj_drop=0.1,
                 attn_drop=0.1, act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
...
class  Spatial_Channel_FFN(nn.Module):
    ...
    def __init__(self, dim, num_heads, split_size=[8,16], expansion_factor=2,
        ...
        self.attn1 = Adaptive_Spatial_Attention(
            ...
            drop=drop_path, attn_drop=attn_drop)
...
class ResidualGroup(nn.Module):
    ...
    def __init__(   self,
        ...
        self.conv = nn.Sequential(nn.Conv2d(dim, dim, 3, 1, 1), nn.BatchNorm2d(dim), nn.LeakyReLU(inplace=True))
...
class DAT(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_init_edge = nn.Sequential(nn.Conv2d(embed_dim, 1, 3, 1, 1),nn.BatchNorm2d(1),nn.Sigmoid())
        ...
        self.conv_after_body = nn.Sequential(nn.Conv2d(embed_dim, embed_dim, 3, 1, 1), nn.BatchNorm2d(embed_dim), nn.LeakyReLU(inplace=True))
        self.conv_then = nn.Sequential(nn.Conv2d(embed_dim, num_feat, 3, 1, 1), nn.BatchNorm2d(num_feat), nn.LeakyReLU(inplace=True))
        self.conv_last = nn.Sequential(nn.Conv2d(num_feat, num_out_ch, 3, 1, 1), nn.BatchNorm2d(num_out_ch), nn.Sigmoid())
