...
class ResidualGroup(nn.Module):
    ...
    def __init__(   self,
        ...
        self.conv_dim2 = nn.Sequential(nn.Conv2d(dim*2, dim*2, 3, 1, 1), nn.BatchNorm2d(dim*2), nn.LeakyReLU(inplace=True))
        self.conv_dim4 = nn.Sequential(
    ...
    def forward(self, msf, fu, pan, x_size):
        ...
        blk5 = self.blocks3[2]
        blk9 = self.blocks3[3]
        blk10 = self.blocks3[4]
        blk11 = self.blocks3[5]
        ...
        #z3(B, C, H, W)z6(B, C*2, H/2, W/2)z9(B,C*4,H/4,W/4)z12(B, C*2, H/2, W/2)

        z3 = z3.transpose(-2,-1).contiguous().view(B, C, H, W)
        z3 = self.up_dim2(z3)#通道数翻倍,尺寸减倍
        ...
        z90 = z9
        z7 = z7.transpose(-2,-1).contiguous().view(B, C*4, H2, W2)
        z7 = self.down_dim2(z7)#通道数减倍,尺寸加倍
        z7 = z7.permute(0, 2, 3, 1).contiguous().view(B, L1, C*2)

        z8 = z8.transpose(-2,-1).contiguous().view(B, C*4, H2, W2)
        z8 = self.down_dim2(z8)#通道数减倍,尺寸加倍
        z8 = z8.permute(0, 2, 3, 1).contiguous().view(B, L1, C*2)

        z9 = z9.transpose(-2,-1).contiguous().view(B, C*4, H2, W2)
        z9 = self.down_dim2(z9)#通道数减倍,尺寸加倍
        z9 = z9.permute(0, 2, 3, 1).contiguous().view(B, L1, C*2)

        z10 = blk9(z9, z7, x1_size)
        z11 = blk10(z10, z8, x1_size)
        z12 = blk11(z11, z9, x1_size)

        
        z12 = rearrange(z12, "b (h w) c -> b c h w", h = H1, w = W1)
        mse_loss = nn.MSELoss()
        reconstruction_loss = mse_loss(z60, z12)
        #print(z9.shape)#torch.Size([10, 192, 16, 16])"""


        z90 = rearrange(z90, "b (h w) c -> b c h w", h = H2, w = W2)
        z90 = self.conv_dim4(z90)
        z90 = rearrange(z90, "b c h w -> b (h w) c")

        return z90, reconstruction_loss
...
class DAT(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_init_edge = nn.Sequential(nn.Conv2d(embed_dim,num_feat//2, 3, 1, 1),nn.BatchNorm2d(num_feat//2),nn.LeakyReLU())
    ...
    def forward_features(self, msf, fu, pan):
        ...
        ##核心关键！！
        x, reconstruction_loss = self.layer(msf, fu, pan, x_size)
    ...
    def forward(self, msf, fu, pan):
        ...
        msf_init = self.conv_init_edge(msf)#(B,30,H,W)
        ...
        pan_init = self.conv_init_edge(pan)#(B,30,H,W)

        x, reconstruction_loss = self.forward_features(msf, fu, pan)
        x = self.conv_after_body9(x)
        x = self.conv_then(x)
        x = self.conv_last(x)
        return x, msf_init, pan_init, reconstruction_loss
...
if __name__ == '__main__':
    ...
    Output, msf_init, pan_init,loss = model(msf, fu, pan)
    #torch.Size([1, 4, 32, 32])
    print(Output.shape, msf_init.shape, pan_init.shape, loss)
"""