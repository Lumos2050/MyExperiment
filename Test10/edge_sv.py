...
class ResNet18(nn.Module):
    ...
    def __init__(self, inplane):
        ...
        self.blk1_2 = ResBlk(inplane, inplane, stride=1)
        self.blk2_2 = ResBlk(inplane, 128, stride=2)
        self.blk3_2 = ResBlk(128, 256, stride=1)
        self.blk4_2 = ResBlk(256, 512, stride=2)
        self.outlayer1 = nn.Linear(1024, 11)
    ...
    def forward(self, x, y):
        ...
        #print(x.size())
        s = torch.cat([x, y], 1)   # 通道拼接
        s = s.view(s.size()[0],  -1)
        #print('ssss', x.shape)
        s = F.relu(self.outlayer1(s))
...
class SqueezeBodyEdge(nn.Module):
    ...
    def __init__(self, inplane):
        ...
        self.conv_5to4 = nn.Sequential(
            nn.Conv2d(inplane + inplane, inplane, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(inplane),
            nn.ReLU(inplace=True))
...
#init的归一化在canny函数里面
        ...
        #print("边缘监督中的释放显存2")
        torch.cuda.empty_cache()
        cls_prd = self.fc(final,weighted_image)
