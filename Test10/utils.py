...
#定义训练方法
def train_model(model, train_loader, optimizer, epoch, lamda1=0.01, lamda2=0.01, lamda3 = 0.02):
    ...
    for step, (ms, fu, pan, label, _) in enumerate(loop):
        ...
        optimizer.zero_grad()
        output, mse_msf, mse_pan, reconstruction_loss = model(ms, fu, pan)
        pred_train = output.max(1, keepdim=True)[1]
        correct += pred_train.eq(label.view_as(pred_train).long()).sum().item()
        mse_msf = torch.tensor(mse_msf).to(torch.float32).cuda().mean()
        mse_pan = torch.tensor(mse_pan).to(torch.float32).cuda().mean()
        reconstruction_loss = torch.tensor(reconstruction_loss).to(torch.float32).cuda().mean()
        loss = F.cross_entropy(output, label.long()) + lamda1 * mse_msf + lamda2 * mse_pan + lamda3 * reconstruction_loss
        loss.backward()
        optimizer.step()
        ...
        if step % 100 == 0 or step == len(loop) - 1:
            ...
            print(f"Train Epoch: {epoch} \t  ce:{F.cross_entropy(output, label.long())}\t msf:{mse_msf}\t pan:{mse_pan}\t recon:{reconstruction_loss}\t Step: {step} \t Train Accuracy: {accuracy:.2f}%")
