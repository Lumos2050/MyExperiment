...
class ResidualGroup(nn.Module):
    ...
    def forward(self, msf, fu, pan, MT, PT, x_size):
        """
...
class DAT(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_init_edge4 = nn.Sequential(nn.Conv2d(4, embed_dim//2, 3, 1, 1),nn.BatchNorm2d(embed_dim//2),nn.LeakyReLU())
        self.conv_init_edge1 = nn.Sequential(nn.Conv2d(1, embed_dim//2, 3, 1, 1),nn.BatchNorm2d(embed_dim//2),nn.LeakyReLU())
    ...
    def forward_features(self, msf, fu, pan, MT, PT):
        _, _, H, W = msf.shape
        x_size = [H, W]
        ...
        ##核心关键！！
        x = self.layer(msf, fu, pan,  MT, PT, x_size)
    ...
    def forward(self, msf, fu, pan):
        ...
        msf_init = self.conv_init_edge4(msf)#(B,C,H,W)
        ...
        pan_init = self.conv_init_edge1(pan)#(B,C,H,W)

        x = self.forward_features(msf_in, fu, pan_in, msf_init, pan_init)
        x = torch.concat([x, msf_init, pan_init], dim=1).float()
        x = self.fc(x)
        #conv_after_body9(x)
        #x = self.conv_then(x)
        #x = self.conv_last(x)
